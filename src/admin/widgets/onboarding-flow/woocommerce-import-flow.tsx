...
const WooImportFlow = (props: OnboardingWidgetProps) => {
  ...
  const isProductImportStep = useMemo(() => {
    return (
      currentStep === "import_products"
    );
  }, [currentStep]);

  const isOrderImportStep = useMemo(() => {
    return (
      currentStep === "import_orders"
    );
  }, [currentStep]);
  ...
  }, [location.pathname, isProductImportStep]);
  ...
  }, [location.pathname, isOrderImportStep]);
