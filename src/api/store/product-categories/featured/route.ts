...
export const GET = async (
    ...
    let results = expand ? await categoriesService.listAndCount({}, { take: parseInt(`${limit || '9999'}`), skip: parseInt(`${offset || '0'}`), relations: [`${expand}`] }) : await categoriesService.listAndCount({}, { take: parseInt(`${limit || '9999'}`), skip: parseInt(`${offset || '0'}`) })
    let categories = []
    results[0].forEach(element => {
        categories.push({
            ...
            is_featured: element?.metadata?.is_featured === "featured" ? true : false,
    ...
    categories = categories.filter(a => a.is_featured).sort(function (a, b) {
